...
void v_add_optimized_adjacent(double* x, double* y, double* z) {
    ...
    #pragma omp parallel
    {
        ...
        for(int i = thread_identifier; i<ARRAY_SIZE; i = i + number_of_threads)
        {
            z[i] = x[i] + y[i];
        }
    }
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
    // TODO: Modify this function
    // Do NOT use the `for` directive here!
    //Distribute threads over the array
    #pragma omp parallel
    {
        int thread_identifier = omp_get_thread_num();
        int number_of_threads = omp_get_num_threads();
        ...
        for(int i = thread_identifier*size; i<(thread_identifier+1) *size;i++)
        {
            z[i] = x[i] + y[i];
        }

        int leftOver = ARRAY_SIZE % number_of_threads;
        if (leftOver != 0)
        {
            for(int i = ARRAY_SIZE-leftOver;i<ARRAY_SIZE;i++)
            {
                z[i] = x[i] + y[i];
            }
        }
